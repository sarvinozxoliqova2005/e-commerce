  ...
  return (
   <>
      ...
      <div className="container mx-auto px-5 space-y-10">
        ...
        <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory flex justify-between pb-2 scrollbar-hide">
          {categories.map((el) => (
            <div
              key={el.id}
              ...
              className={`
                ...
                p-4
                cursor-pointer
                ${categoryId === el.id ? "" : "bg-white border-white"}
              `}
            >
              {el.icon && <img src={el.icon} alt={el.title} className="w-14 h-14 object-contain" />}
        ...
        >
          {[
            ...
            <div
              key={index}
              className="snap-start min-w-[260px] bg-white rounded-2xl shadow-2xl border-2 border-white p-4 flex flex-col hover:scale-105 duration-500"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5 px-4">
            {products
              ...
              .map((p) => (
                <div
                  key={p.id}
                  className="bg-white rounded-2xl shadow-2xl border-2 border-white p-4 flex flex-col hover:scale-105 duration-500"
                >
                  <img
                    src={p.image}
                    alt={p.title}
                    className="w-full max-w-[500px] h-40 object-contain mb-3"
                  />
