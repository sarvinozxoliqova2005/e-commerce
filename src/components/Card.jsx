...
const Card = ({ image, title, basePrice, weight, id, badge }) => {
  ...
  return (
    <div className="snap-start relative min-w-[100%] sm:min-w-[260px] bg-white rounded-2xl shadow-lg border-2 border-white p-4 flex flex-col hover:scale-105 duration-500">
      ...
      {badge && (
        <span className="absolute top-2 right-2 w-15 h-8 bg-orange-500 text-white rounded-lg flex items-center justify-center text-xs font-semibold">
          {badge}
        </span>
      )}
      ...
      {itemInCart ? (
        <div className="flex items-center justify-center gap-2 mt-3">
          <button
            ...
            className="bg-orange-500 text-white w-full py-[10px] rounded-lg font-semibold"
          >
            -
          </button>
        ...
        <button
          onClick={() => addToCart(id)}
          className="flex-1 bg-orange-500 cursor-pointer text-white py-2 rounded-lg font-semibold hover:bg-orange-600 transition mt-3"
        >
          Выбрать
        </button>
      )}
